...
**Phase:** 0.1 — measurement and visibility only. **No enforcement of any kind: no overlays, no blocking, no package suspension.**
...
**Explicit non-goals for 0.1:** AccessibilityService blocking, overlay windows, `setPackagesSuspended`, device-owner provisioning automation, kiosk features. 0.1 *may* include device-owner **detection** (see §9) but must not act on it.
...
A status screen shows each permission green/red with a fix button — this screen doubles as the tamper-visibility surface in 0.1: if usage access is revoked, the app says so openly and reports degraded state on its next heartbeat (see §9).
...
- **Media signal:** `MediaSessionManager.getActiveSessions()` (requires notification listener permission — **request optionally, not required**) and `AudioManager.isMusicActive()` as a permission-free fallback. Rationale per category: `media_counts_as_active` governs whether an app that is foreground-but-possibly-idle (video apps) keeps counting. Without the media signal, count foreground-while-screen-on time for all categories (documented 0.1 simplification on Android).
- 0.1 Android counting rule, explicit: `counting = screen_on AND app_foreground AND app_mapped`
...
- Multi-user devices: 0.1 assumes the app runs in the kid's profile only; document that Android guest/secondary profiles bypass tracking entirely (parent setup responsibility)
...
- `EnforcementController` interface (`onCategoryExhausted(category)`, `onLockdown(state)`) with a no-op 0.1 implementation, subscription to `binary_sensor.gnomon_exhausted_*`/`switch` states behind a default-off flag
